...
//    }
    ...
    public void topic_one(ConsumerRecord<?, ?> record, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic,Acknowledgment ack){

        Optional message=Optional.ofNullable(record.value());
        if (message.isPresent()){
            Object msg=message.get();
            logger.info("被"+"demo"+"消费了： +++++++++++++++ Topic:" + topic+",Record:" + record+",Message:" + msg);
//            ack.acknowledge();
        }
    }

    @KafkaListener(topics = "iris",groupId="group-one")
    public void topic_two(ConsumerRecord<?, ?> record, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic,Acknowledgment ack){
